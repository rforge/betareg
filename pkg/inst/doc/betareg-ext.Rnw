...
variance.  In this parameterization the beta distribution
has the density
%
...
Hence, iteration provides a unified framework for implementing all three types of
...
beta regression where where substantial upwards bias was
...
\fixme{indicate that this is for BG?}
