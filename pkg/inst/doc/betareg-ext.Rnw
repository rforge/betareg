...
\code{x2}, and \code{x3} to the mean specification~\eqref{eq:link1}
and \code{z1} and \code{z2} to the precision
specification~\eqref{eq:link2}, respectively.  Function \fct{betareg}
...
for the precision parameters as shown in expression~\ref{eq:link2}.
...
\end{align*}
with $\alpha_1 \equiv 0$. {\color{blue} If $c = 1$, it just contains a constant and this reduces
...
anchor would be assumed {\color{blue} at position $1/K$} if the
...
each child, the variables \code{accuracy} (the score on a test of
reading accuracy), \code{iq} (the score on a nonverbal intelligent
quotient test, converted to $z$ score), and a binary variable on
whether the child is dyslexic or not were recorded.
% the scores on a test of reading accuracy (variable
% \code{accuracy}) as well as on a nonverbal intelligent quotient
% (variable \code{iq}, converted to $z$~scores) as well as if the child
% is dyslexic or not (variable \code{dyslexic}) are reported.
...
illustrate how {\color{blue} (a)~the dyslexia groups would also have been found
by recursive partitioning if dyslexia had just been one of many
covariates and how (b)~mixture modeling recovers the dyslexia groups
if that covariable had not been known at all.}
...
\caption{\label{tab:ReadingSkills-bias} Comparison of coefficients and
  standard errors (in parentheses) in interaction model for reading
  ...
  very similar results for the mean specification. In the precision
  specification, main effects are slightly damped and the interaction
...
give very similar results for the mean specification. In the precision
specification, main effects are slightly damped and the interaction
...
effect of BC/BR would become even smaller when the model without
interaction in the precision specification (\citealt[as chosen
by][]{betareg:Smithson+Verkuilen:2006}) was considered.
...
a beta regression where both the mean and the precision depend on
...
the mean and the precision. {\color{blue} Hence the results are
...
only guaranteed to converge a local optimum and the convergence
...
In contrast, the group of nondyslexic children is modelled
differently. With observed dyslexia, the heterogeneity in the control
group is captured by differences in the precision specification, i.e.,
...
maximum likelihood estimator. Recursive partitioning methods and
...
correction/reduction is used, indicating a large upwards bias for the
ML estimator of $\phi$.
%
<<GasolineYield-phi>>=
