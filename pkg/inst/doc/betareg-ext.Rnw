...
\Abstract{
Beta regression -- an increasingly popular approach for modeling rates and proportions --
is extended in various directions: (1)~bias correction/reduction of the maximum
likelihood estimator, (2)~beta regression tree models by means of recursive partitioning,
(3)~latent class beta regression by means of finite mixture models. All three
extensions may be of importance for enhancing the beta regression toolbox in practice
to provide more reliable inference and capture both observed and unobserved/latent
heterogeneity in the data. Using the analogy of \cite{betareg:Smithson+Verkuilen:2006},
these extensions make beta regression not only ``a better lemon squeezer'' (compared to classical
least squares regression) but a full-fledged modern juicer offering lemon-based drinks:
shaken and stirred (bias correction and reduction), mixed (finite mixture model),
or partitioned (tree model). All three extensions are provided in the \proglang{R} package
\pkg{betareg} (at least 2.4.0), building on generic algorithms and implementations
for bias correction/reduction, model-based recursive partioning, and finite mixture
models, respectively. Specifically, the new functions \fct{betatree} and \fct{betamix}
reuse the object-oriented flexible implementation from the \proglang{R} packages
\pkg{party} and \pkg{flexmix}.
}
...
parametrization of the beta distribution characterizing more easily the mean
and the variance.  In this parameterization the beta distribution
has the density
%
...
(\beta_1, \ldots, \beta_p)^\top$, $\gamma = (\gamma_1, \ldots, \gamma_q)^\top$
are the vectors of parameters associated with the means and the
precisions, respectively. The functions $g_1(.)$ and $g_2(.)$ are monotonic link
functions, preferably with the property of mapping the range of $\mu_i$
$(0, 1)$ and $\phi_i$ $(0, \infty)$, respectively, to the real line.
Suitable candidates for $g_1(\cdot)$ are the
...
choice for $g_2(\cdot)$ is the identity function which, however,
may lead to invalid results if not all $\zeta_i > 0$.
...
The \proglang{R} package \pkg{betareg} \citep{betareg:Cribari-Neto+Zeileis:2010}
provides ML estimation of beta regressions in its main
model fitting function \fct{betareg}. The interface as well as the
fitted model objects are designed to be similar to those from \fct{glm}. The
...
allowed \citep[by means of \pkg{Formula},][]{betareg:Zeileis+Croissant:2010},
e.g., \code{y ~ x1 + x2 + x3 | z1 + z2}, where the covariates
\code{x1}, \code{x2}, and \code{x3} pertain to the mean equation~\eqref{eq:link1} and
\code{z1} and \code{z1} to the precision equation~\eqref{eq:link2}, respectively.
Function \fct{betareg} internally uses function \fct{optim} as a general purpose optimizer to
...
Although the \pkg{betareg} package as published by \cite{betareg:Cribari-Neto+Zeileis:2010}
provides a rather complete beta regression toolbox based on classical
ML inference, further techniques may be required in practice. First,
it has been shown that ML inference may be severely biased in the
context of beta regression \citep{betareg:Kosmidis+Firth:2010}, possibly
leading to overly optimistic effects. Second, it is not always easy
to capture all heterogeneity in the data through the two linear predictors,
especially when there are latent unobserved groups/clusters of observations.

To address the first issue of potentially biased inference, the results
of \cite{betareg:Kosmidis+Firth:2010} are extended to the case with
both linear predictors (and not only one for the mean) and implemented
in the \fct{betareg} function starting from version~2.4-0 of \pkg{betareg}.
The software optionally allows for bias-corrected or bias-reduced ML
estimation by adopting the unifying iteration developed in
\cite{betareg:Kosmidis+Firth:2010}.

To address the second issue of heterogeneity between groups/clusters
of observations, two generic strategies, model-based recursive partitioning
\citep{betareg:Zeileis+Hothorn+Hornik:2008} and finite mixture models
\citep[see e.g.,][]{betareg:McLachlan+Peel:2000, betareg:Fruehwirth-Schnatter:2006},
are applied to beta regressions. The idea for both techniques is to
capture situations in which the regression relationships vary across groups in the population.
If variables can be identified which are directly related to these groups, 
one may be able to include them directly in the regression. However,
(1)~this may lead to rather complex and hard to interpret models,
and (2)~unnecessary complexity is introduced if the differences are only
present in a subset of the combined groups induced by several variables.
Model-based recursive partitioning is an alternative approach which avoids these
drawbacks. Furthermore, if groups cannot be directly related to observed
variables, the heterogeneity can be accounted for using finite mixture
models. Therefore, extensions of the \pkg{betareg} package are introduced
...
the heterogeneity is due to latent variables. The new function \fct{betatree}
provides model-based recursive partitioning of beta regressions 
leveraging tools from the \pkg{party} package \citep{betareg:Hothorn+Hornik+Strobl:2011}
and function \fct{betamix} provides beta regression mixture models
(or latent class beta regression) reusing the generic functionality
from the \pkg{flexmix} package \citep{betareg:Leisch+Gruen:2011}.
...
bias reduction (BR) of the ML estimator in
parametric models may be achieved via a unifying \emph{quasi} Fisher
scoring algorithm. They illustrate the
applicability of their algorithm in a beta regression setting with a
common dispersion parameter $\phi$ for all subjects, also revealing
some errors in previous literature for the reduction of bias in beta
regression models -- specifically mistakes in
\cite{betareg:Ospina+Cribari-Neto+Vasconcellos:2006} and
\cite{betareg:Simas+Barreto-Souza+Rocha:2010} that led to
misleading negative conclusions about the effect of BC/BR
on inferences for beta regression models. In \cite{betareg:Kosmidis+Firth:2010},
it is shown that BC/BR for beta regression models can be
desirable because the ML estimator of $\phi$
may demonstrate substantial upwards bias, which in turn may lead to
underestimation of asymptotic standard errors and hence
over-optimistic Wald-type inferences (e.g., confidence
intervals with coverage far below the nominal levels).
...
dispersion parameters as shown in Equation~\ref{eq:link2}.
...
p + q) \, ,
\]
where $F(\theta)$ is the expected information matrix and
\begin{eqnarray}
...
If the summand $b\left(\theta^{(j)}\right)$ is ignored, then
iteration \eqref{eq:iteration} becomes the usual Fisher scoring
iteration that can be used to solve the ML score
...
Hence, the quasi Fisher scoring iteration provides a unified framework for implementing all three types of
estimators -- ML, BC, and BR -- by merely
deciding whether the summand $b\left(\theta^{(j)}\right)$ is absent or
present in the right hand side of \eqref{eq:iteration}, and whether
more than one iteration should be allowed in the latter case.


\subsection{BC/BR for beta regressions}

Denote the vector of all model $p + q$ parameters in a beta regression by $\theta =
(\beta^\top, \gamma^\top)^\top$, and let $X$ and $Z$ be the $n\times
p$ and $n\times q$ regressor matrices with $i$-th row $x_i$ and $z_i$, respectively.
The ingredients required for setting the
iteration described in Section~\ref{sub:iteration} are closed-form
...
$Q_t(\theta)$ shown in \eqref{eq:P} and \eqref{eq:Q}. Based on these, all matrix
...
Differentiating $\ell(\theta)$ one more time gives that the observed
...
centred sufficient statistics and hence has expectation zero. Here,
...
respectively. The argument \code{type} controls which of the
maximum likelihood (\code{type = "ML"}), bias-corrected (\code{type = "BC"}),
or bias-reduced (\code{type = "BR"}) estimates is computed.
Finally, \code{control} is a list of control arguments and \code{model},
...
$b\left(\theta^{(j)}\right)$ in iteration \eqref{eq:iteration}.
Thus, the iteration is either used to further improve the
numerical maximization of the likelihood (for \code{type = "ML"} or
\code{type = "BC"}) or to carry out the bias reduction (for
\code{type = "BR"}) as detailed in Section~\ref{sub:iteration}. To control
...
controlling the maximal number of iterations and convergence
...
step~2). Here, the objective is the log-likelihood $\ell(\theta)$ and its
contributions $\ell_i(\theta)$ are given in \eqref{eq:loglik} . By
...
nonnegative for all $k$ and sum to one. In the following they are
...
fact that the complete-data log-likelihood for the data as well as the missing
...
E-step. The M-step then consists of maximising the complete-data
log-likelihood where the missing component memberships are replaced by the
...
In the following we reconsider an analysis of reading accuracy data
...
are given in \cite{betareg:Cribari-Neto+Zeileis:2010}. Here, we
provide their model visualizations in Figure~\ref{fig:ReadingSkills}
to briefly highlight the most important findings: In the control group,
reading skills increase clearly with IQ score while variance clearly decreases.
In the dyslexic group, reading skills are generally lower and almost
unaffected by IQ score.
...
Subsequently, it is first assessed whether the ML inference results in this
analysis may have been affected by bias. Subsequently, it is illustrated
how the differences with respect to dyslexia could have been discovered in
a data-driven way. While in the original study dyslexia has, of course, been of prime
interest in the model, the data set is used here to illustrate how (a)~the dyslexia
groups would also have been found by recursive partitioning if dyslexia had just been
one of many covariates and how (b)~mixture modeling recovers the dyslexia
groups if that covariable had not been known at all.


\subsection{Bias correction and reduction}

To investigate whether the results of \cite{betareg:Smithson+Verkuilen:2006}
may have been affected by biased inference, all three flavors of estimators
are obtained for the model with interactions (both in
...
\texttt{rs\_ml}, the BC estimator from \texttt{rs\_bc}, and the BR estimator from \texttt{rs\_br} all give very similar results for
the mean equation. In the precision equation, main effects are slightly damped and the interaction
effect slightly amplified when using BC/BR.}
\end{table}
...
@
\caption{\label{fig:readingskillsbias} Scatterplots of the logarithm of
  the estimated precision parameters $\log(\phi_i)$ based on the
  maximum likelihood, bias-corrected and bias-reduced estimate.}
\end{figure}


The resulting coefficient estimates, standard errors, and log-likelihood
can be displayed using the \fct{summary} method and are reported
in Table~\ref{tab:ReadingSkills-bias}. All three
...
interaction effect slightly amplified when using
BC/BR. Figure~\ref{fig:readingskillsbias} shows the
scatter plots of the logarithm of the estimated precision parameters
based on the ML, BC, and BR estimates. It is apparent that the logarithms of the
...
\cite{betareg:Kosmidis+Firth:2010}. The reason that
the effect is milder in this particular example relates to the fact
that bias for the precision parameters is corrected/reduced on the
log-scale where the ML estimator has a more symmetric
distribution than on the original scale. To emphasize that BC/BR
may potentially be crucial for empirical analyses, Appendix~\ref{sec:gasoline}
replicates the results of \cite{betareg:Kosmidis+Firth:2010} for a
beta regression where substantial upwards bias was
detected for the ML estimator of the precision
...
on the precision parameters in \eqref{eq:expinfo}.)
...
accuracy does not depend on the noise variables which we would
...
fitted to each of the two groups of children are illustrated in the
...
In each node the parameter instabilities for the partitioning
variables are determined if the resulting node size is sufficiently
large. The test statistics together with the corresponding $p$ values
can be obtained using function \fct{sctest}. This indicates if more
than one partitioning variable indicated significant instability and
the reason for performing no further split, i.e., either because all
...
variables are all nonsignificant. In node~2, dyslexia cannot be used
for splitting anymore and all other variables are still
nonsignificant.  With only 19~observations, node~3 is considered too
small to warrant further splitting given that \code{minsplit=10}
requires that each node contains at least 10 observations and hence no
tests are carried out.

Function \fct{coef} allows to inspect the parameters of the fitted
models in the terminal nodes (nodes~2 and 3). 
...
The group of nondyslexic children is challenging as it consists of two
subgroups -- one with essentially perfect accuracy and one with
strongly increasing accuracy by IQ score. In a model with observed
\code{dyslexia}, this can be captured by different variances in the
two groups. However, if \code{dyslexia} is unobserved and a
mixture model is employed to infer the groups, then this is
problematic. The subgroup with perfect reading score will typically be
selected as one component where the variance converges to zero leading
to an unbounded likelihood. To address this we fit a finite mixture
...
probability. Furthermore it is indicated if the EM algorithm converged
...
shading is in red the higher is the a-posteriori probability for this
observation to be from component 1. Blue shading corresponds to the
...
similar as indicated by the comparison of the two red lines. By
contrast the group of nondyslexic children is modelled
differently. With observed dyslexia, the heterogeneity in the control
group is captured by differences in the precision equation, i.e., in
the variance. However, for unobserved dyslexia, it is more appropriate
to capture the increased heterogeneity in the control group by two
components, one of which corresponds to perfect reading accuracy
independent of IQ score.
...
The new extensions of the package \pkg{betareg} allow to go beyond
classical ML inference when fitting beta regression models. Bias
correction / reduction of the ML estimates helps in performing more
reliable inference. Recursive partitioning methods and finite mixture
models enable the user to investigate heterogeneity -- observed as well
as unobserved -- in the regression model fitted to the whole sample.

For users already familiar with the previous version of the
\pkg{betareg} package obtaining the bias corrected / reduced
estimators does not require them to learn about any new functions,
only the \code{type} argument when fitting the model using
\fct{betareg} needs to be modified. To fit beta regression trees as
well as finite mixtures of beta regressions two new functions are
available in package \pkg{betareg}. These functions reuse
functionality from packages \pkg{party} and \pkg{flexmix}. Their
interface is aimed to be as similar as possible to \fct{betareg} in
order to facilitate the use for those who are already familiar with
package \pkg{betareg}.

The implementation of the extensions required changes in the fitting
function \fct{betareg.fit} in order to allow quasi Fisher scoring
iterations to follow ML estimation using \fct{optim}. This
functionality can now be either used to obtain improved ML estimates or
the bias corrected / reduced versions of the ML estimates. For
modelling heterogeneity \fct{betareg.fit} is reused to fit the models
in the nodes in the case of beta regression trees and in the M-step
for finite mixture models. Only a small amount of additional code,
e.g., \proglang{S4} glue, was required to combine package
\pkg{betareg} with packages \pkg{party} and \pkg{flexmix}.

The increased flexibility of the extended package \pkg{betareg}
enables users to conveniently check model suitability as well as
appropriateness of their inference. With the extended package they can
easily compare the results from an ordinary beta regression model
fitted using ML estimation to those using bias correction / reduction
as well as when allowing for observed or unobserved
heterogeneity. This will allow them to choose a suitable model as well
as increase the confidence in the finally selected fitted model and
...
for fixed-precision beta regression models with identity link and different types of estimators on gasoline yield data.}
\end{table}
%
...
@
%
This results in much larger standard errors (and hence smaller test statistics
and larger $p$~values) for all coefficients in the mean equation of the model,
see Table~\ref{tab:GasolineYield-bias} that replicates \citet[Table~1]{betareg:Kosmidis+Firth:2010}.
The picture does also not change much when a log-link is used in the precision model,
see below and Table~\ref{tab:GasolineYield-bias2} replicating \citet[Table~3]{betareg:Kosmidis+Firth:2010}.
...
\end{tabular}
\caption{\label{tab:GasolineYield-bias2} Coefficient estimates and standard errors (in parentheses)
for fixed-precision beta regression models with log-link and different types of estimators on gasoline yield data.}
\end{table}
