...
$\VAR(y) = \mu(1-\mu)/(1+\phi)$. It has density
%
...
][]{betareg:Breiman+Friedman+Olshen:1984}. As for CART, the idea
is to split the sample recursively with respect to available variables
(called \dquote{partitioning} variables in the following) in order to capture
differences in the response variable. While CART tries to capture differences
in the distribution of the response variable (in particular with respect to
location) directly, the aim of model-based recursive partitioning
\citep[MOB,][]{betareg:Zeileis+Hothorn+Hornik:2008} is more broadly to 
capture differences in a parameters describing the distribution of the response.
...
stopping criterion is met. Thus, the interactions and nonlinearities can
...
regression are the following, where $c_ij$ is the $j$-the partitioning
...
\begin{enumerate}
  ...
  \item If there is significant parameter instability with respect to
        ...
        sample along the $c_{ij^*}$ with the strongest association:
  ...
  \item Repeat steps 1--3 recursively in the resulting subsamples
        until there is no more significant instability or the
...
and the minimal subsample size in step~4 -- the \code{...} to passed to \fct{mob}. (Note
