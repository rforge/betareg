...
\Abstract{Beta regression -- an increasingly popular approach for
  ...
  i) bias correction/reduction of the maximum likelihood estimator,
  ii) beta regression tree models by means of recursive partitioning,
  iii) latent class beta regression by means of finite mixture
  ...
  the \proglang{R} packages \pkg{party} and \pkg{flexmix}.  }
...
to be distinguished a two part formula is allowed by using
...
\code{x2}, and \code{x3} to the mean expression \eqref{eq:link1} and
\code{z1} and \code{z2} to the precision expression \eqref{eq:link2},
...
population.  If variables can be identified which are directly related
to these groups, one may be able to include them directly in the
regression. However, i) this may lead to rather complex and hard to
interpret models, and ii) unnecessary complexity is introduced if the
...
dispersion parameters as shown in expression~\ref{eq:link2}.
...
by i) fitting a beta regression model, ii) assessing whether its
parameters are stable across all partitioning variables, iii)
splitting the sample along the partitioning variable associated with
the highest parameter instability, iv) repeating these steps until
...
partitioning variable ($j = 1, \dots, p$) for observation $i$, the
...
models the a-posteriori probabilities of an observation to be from
each component given the current parameter estimates are determined in
the E-step. The M-step then consists of maximising the complete-data
log-likelihood where the missing component memberships are replaced by
the a-posteriori probabilities. This implies that different mixture
...
obtained for the model with interactions (both in the mean and
precision equations).
...
original scale. To emphasize that BC/BR may potentially be crucial for
...
would not be influenced by bias-related issues.
...
IQ score. In order to assess whether being dyslexic has an influence
on the relationship between \code{accuracy} and \code{iq}, the
variable \code{dyslexia} is used as a partitioning variable. For
investigating the ability of the method to select suitable variables
for partitioning, additional random noise variables are added to the
list of partitioning variables. One noise variable is drawn from a
...
least converge to a local optimum and the initialization already
...
shaded according to the magnitude of the corresponding a-posteriori
probabilities. The stronger the shading of an observation is in red,
the higher the a-posteriori probability for this observation being
...
the dyslexic and nondyslexic children of the model where an
interaction with the variable \code{dyslexic} is specified in the
regressions for mean and precision are provided. These mean regression
lines are equivalent to those obtained using model-based recursive
partitioning.
...
allows to check how sensitive inferences (standard errors) can be to
...
does not require the use of any new function; the user only needs to
appropriately specify the \code{type} argument (which defaults to
\code{"ML"}).
...
scoring iteration is accordingly modified, as described in
Subsection~\ref{sub:iteration} .
