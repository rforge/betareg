...
\Abstract{
  ...
  the \proglang{R} packages \pkg{partykit} and \pkg{flexmix}, respectively.
}
...
%\VignetteDepends{stats,betareg,partykit,flexmix}
...
partitioning of beta regressions leveraging tools from the \pkg{partykit}
package \citep{betareg:Hothorn+Zeileis:2015}, and the function
...
respectively. The formula \code{y ~ c1 + c2 + c3} is short for
\code{y ~ 1 | 1 | c1 + c2 + c3}.
...
\pkg{partykit} package
\citep{betareg:Hothorn+Zeileis:2015}. The
...
and for extractions the \fct{logLik} and \fct{estfun} methods \citep[see
...
@
%
The returned object is of class \code{"\Sexpr{class(rs_tree)[1]}"} which
inherits from \code{"\Sexpr{class(rs_tree)[2]}"} and \code{"\Sexpr{class(rs_tree)[3]}"}.
All methods for \code{"\Sexpr{class(rs_tree)[2]}"}/\code{"\Sexpr{class(rs_tree)[3]}"}
objects can be reused, e.g., the \fct{print} method and the \fct{plot} method (see
...
packages \pkg{partykit} and \pkg{flexmix}. The interface of the two new
...
necessary to inherit the functionality provided by the \pkg{partykit} and
