...
power <- function(lambda = 1)
{
    ...
    dmu.deta <- function(eta)
        pmax((1/lambda^2) * eta^(1/lambda - 2) * (1 - lambda), .Machine$double.eps)
...
make.link <- function (link)
{
    switch(link,
           ...
           "cloglog" = {
               ...
               dmu.deta <- function(eta) {
                 pmax((1 - exp(eta)) * exp(eta) * exp(-exp(eta)), .Machine$double.eps)
               }
               valideta <- function(eta) TRUE
           },
           "identity" = {
...
gaussian <- function (link = "identity")
{
    ...
    structure(list(family = "gaussian",
		   ...
		   aic =	function(y, n, mu, wt, dev) {
                   ...
                   d1afun = function(zeta) -1/zeta,
                   d2afun = function(zeta) 1/zeta^2,
                   d3afun = function(zeta) -2/zeta^3,
		   mu.eta = stats$mu.eta,
                   dmu.deta = stats$dmu.deta,
		   initialize = expression({
		       n <- rep.int(1, nobs)
...
Gamma <- function (link = "inverse")
{
    ...
    structure(list(family = "Gamma",
		   ...
		   dev.resids = dev.resids,
		   aic = aic,
                   d1afun = function(zeta) -2*psigamma(-zeta, 0) + 2*log(-zeta),
                   d2afun = function(zeta) 2*psigamma(-zeta, 1) + 2/zeta,
                   d3afun = function(zeta) -2*psigamma(-zeta, 2) - 2/zeta^2,
		   mu.eta = stats$mu.eta,
                   dmu.deta = stats$dmu.deta,
		   initialize = initialize,
		   validmu = validmu,
