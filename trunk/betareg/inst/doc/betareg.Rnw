...
\Abstract{
  ...
  in the \proglang{R} system for statistical computing and graphics. The
...
\cite{betareg:Ferrari+Cribari-Neto:2004} defined the following \emph{standardized ordinary residual}: 
...
$\hat{\beta}$ and $\hat\phi$ being the maximum likelihood estimators of $\beta$ and $\phi$, respectively. A better residual was proposed by \cite{betareg:Espinheira+Ferrari+Cribari-Neto:2008a}, which they named \emph{standardized residual 2}:
\begin{equation*}
