...
\citep{betareg:Ferrari+Cribari-Neto:2004,betareg:Simas+Barreto-Souza+Rocha:2010}

also \cite{betareg:Ospina+Cribari-Neto+Vasconcellos:2006} and
\cite{betareg:Espinheira+Ferrari+Cribari-Neto:2008}

maybe also \cite{betareg:Kieschnick+McCullough:2003}
...
## dispersion model with regressor temp
...
## Section 5 from Cribari-Neto and Lima (2009)
## log-log link
gy_loglog <- betareg(yield ~ batch + temp, data = GasolineYield, link = "loglog")

## clearly improves AIC
AIC(gy_logit, gy_loglog)
## (apparently the df did not include phi in Cribari-Neto and Lima, 2009)

## pseudo R-squared can not be reproduced for log-log specification
summary(gy_logit)$pseudo.r.squared
summary(gy_loglog)$pseudo.r.squared
## which is
cor(qlogis(GasolineYield$yield), predict(gy_loglog, type = "link"))^2
