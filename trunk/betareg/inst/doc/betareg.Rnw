...
\fixme{Z: Reviewers asked for motivation of beta distribution as a flexible
distribution. Based on your code I have already added the graphic, but the
...
\readme{Z: New choice of starting values in dispersion model, as discussed by e-mail.}
...
\fixme{Z: Note to self, I will change back the nsim argument at the
very end of the revision.}
%
<<GasolineYield-plot, eval=FALSE>>=
set.seed(123)
plot(gy_logit, which = 1:4, type = "pearson")
plot(gy_logit, which = 5, type = "deviance", sub.caption = "", nsim = 1)
...
\readme{Z: Added BIC remark as suggested by reviewer~1.}
%
...
although with a somewhat shrinked \code{income} slope.
...
link have waived the need for a variable precision equation.
...
\readme{Z: Reviewer~1 wanted a different display. This was the best I could think of.}
%
...
\begin{center}
\setkeys{Gin}{width=0.75\textwidth}
<<GasolineYield-diagnostics1, echo=FALSE, fig=TRUE, width=6, height=5.5>>=
...
obtain this for updated version of \code{fe_beta2} with all available link
functions. \readme{Z: This interrupts the reading flow somewhat but
still better than doing everything `by hand' as implied by reviewer~1.}
%
...
whether the model parameters are stable throughout the entire the sample or
...
\readme{Z: I added some more details about structural change methods. See also
below. Hopefully, these give a somewhat clearer picture of what is going on.}
