...
We shall, however, focus on the parameterization that indexed by the mean and a precision
...
that number of trials is large enough to justify a continuous model. In this
...
All of \fct{optim}'s methods are available but the default is to to 
...
\begin{figure}[p!] % \fixme{change to t! in final version?}
\begin{center}
\setkeys{Gin}{width=\textwidth}
